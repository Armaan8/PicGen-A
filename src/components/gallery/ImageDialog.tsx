...
const ImageDialog = ({image, onClose}:ImageDialogProps) => {
  ...
  <SheetContent className='max-w-full sm:max-w-xl w-full'>
    <SheetHeader>
      ...
      <Image src={image.url || "/placeholder.svg"} alt={image.prompt || ""} width={image.width || 0} height={image.height || 0} className='w-full h-auto flex mb-3 rounded' />
      ...
      <div className='flex flex-wrap gap-3 mb-32'>
        ...
        {image.model?.startsWith(`${process.env.NEXT_PUBLIC_REPLICATE_USER_NAME}/`) ? image.model.split("/")[1].split(":")[0] : image.model }
        </Badge>
