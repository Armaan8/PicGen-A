...
const Configurations = ({userModels, model_id}:ConfigurationsProps) => {
    ...
    const form = useForm<z.infer<typeof ImageGenerationFormSchema>>({
        ...
        defaultValues: {
          model: model_id? `${process.env.NEXT_PUBLIC_REPLICATE_USER_NAME}/${model_id}` : "black-forest-labs/flux-dev",
          prompt: "",
          guidance: 3.5,
          num_outputs: 1,
          output_format: "jpg",
    ...
    async function onSubmit(values: z.infer<typeof ImageGenerationFormSchema>) {
        const newValues = {
          ...values, 
          prompt: values.model.startsWith(`${process.env.NEXT_PUBLIC_REPLICATE_USER_NAME}/`) ?  
          
          (() => {

            const modelId = values.model.replace(`${process.env.NEXT_PUBLIC_REPLICATE_USER_NAME}/`, "").split(":")[0];
  ...
  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      <fieldset className="grid gap-6 p-4 bg-background rounded-lg border">
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  ...
                  {
                    userModels?.map(model =>  model.training_status === "succeeded" &&  <SelectItem key={model.id} value={`${process.env.NEXT_PUBLIC_REPLICATE_USER_NAME}/${model.model_id}:${model.version}`}>{model.model_name}</SelectItem>  )
                  }
                </SelectContent>
              </Select>
              <FormMessage />
            ...
            <FormItem>
              <FormLabel className='flex items-center justify-between'>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Quality when sabing the output image, from 0 to 100. 100 is best quality, 0 is lowest quality.</p>
                    </TooltipContent>
                  </Tooltip>
                  </div>
                  <span>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className='flex itemce-center gap-2'>Output Format
              <Tooltip>
                <TooltipTrigger>
                    <Info className="w-4 h-4" />
                </TooltipTrigger>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className='flex itemce-center gap-2'>Prompt
              <Tooltip>
                <TooltipTrigger>
                    <Info className="w-4 h-4" />
                </TooltipTrigger>
